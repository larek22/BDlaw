...
Приложение поднимает полноценный рабочий стол: выберите файлы, заполните всплывающий диалог с метаданными (юрисдикция, код закона, редакция) и индексирование запустится автоматически в фоне. Поиск, формирование ответа GPT и просмотр логов доступны сразу после завершения индексации. Горячие клавиши: `Ctrl+I` (Импорт), `Ctrl+Enter` (Индексировать), `Ctrl+F` (Поиск), `F5` (Тесты).
